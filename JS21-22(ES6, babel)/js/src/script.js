...
let mathObjStr = JSON.stringify(mathTestObj);//преобразовываем js обьект в JSON строку(JSON формат передачи данных от браузера к серверу и обратно)
...
let mathTest = localStorage.getItem('lighttest');//получаем обратно наше значение по ключу с хранилища
mathTest = JSON.parse(mathTest);//превращаем в js обьект (распарсили)

let html = $('#light_test').html();//обьявили переменную и записали html в нее по id все совпавшие элементы
let content =tmpl(html, mathTest);//обьявили переменную и присвоили ей метод tmpl c аргументами html(теги) и распарсенный js обьект тем самым запускаю шаблон
  ...
  function goodResult(){ //функция отрабатывающая появление модального окна при правильном тесте
   ...
   $('#check_result').click(function(){//найденный элемент при клике запускает функцию проверки ответов
    ...
    if (checkedAnswers.length == correctAnswers.length) {//используем логический оператор для сравненияответов на правильность
      ...
      //for(let value of checkedAnswers.length) {
      for(let k =0; k < checkedAnswers.length; k++) {//циклом проходим по ответам
